...
def find_L(psi, pos, error_limit=1.0e-12, maxloop=50):
    old_L = np.identity(psi[pos].shape[0])
    loop_counter = 0
    error = np.inf
...
def find_R(psi, pos, error_limit=1.0e-12, maxloop=50):
    old_R = np.identity(psi[pos-1].shape[2])
    loop_counter = 0
    error = np.inf
...
def P_decomposition(R, L, args, mode='value'):
    ...
    PR = np.dot(R, np.transpose(vh))
    PR = np.dot(PR, re_sq)
    # P_L(n) = 1/sqrt(S(n)) dag(U(n)) L(n)
    PL = np.dot(re_sq, np.transpose(u))
    PL = np.dot(PL, L)
    return PR, PL
"""
...
def find_projector_C4(psi, maxloop=50):
    args = {'cutoff': 1.0e-12, 'bondm': 1024}
    PR_list = []
    PL_list = []
...
"""
def filter_C4(tA):
    tB = tA.transpose(1,2,3,0)
    psi = make_psi(tA,tB)
    PR_list, PL_list = find_projector_C4(psi)
    tA = ncon((tA,PL_list[0],PR_list[0],PL_list[0],PR_list[0]),([1,2,3,4],[-1,1],[2,-2],[-3,3],[4,-4]))
    return tA